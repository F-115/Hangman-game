...
def draw_oval():
    x_origin = config.get_ovalX()
    y_origin = config.get_ovalY()
    radius_x, radius_y = config.get_oval_radii()
    randomR, randomG, randomB = config.get_random_colors()

    glColor3f(randomR, randomG, randomB)

    # Draw an oval (ellipse) using glutSolidSphere
    glBegin(GL_POLYGON)
    for i in range(360):
        angle = i * 2.0 * 3.14159 / 360
        x = x_origin + (radius_x * cos(angle))
        y = y_origin + (radius_y * sin(angle))
        glVertex2f(x, y)
    glEnd()
